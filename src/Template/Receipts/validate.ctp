...
<div class="card-body">
                        ...
                        <tbody>
                            <?php foreach ($receipt->supporderproducts as $key => $supporderproduct): ?>
                                <tr>
                                    ...
                                    <?= $this->Form->control('supporderproducts.' . $supporderproduct->id . '.productunite_id', ['type' => 'hidden', 'class' => 'form-control', 'label' => false, 'value' => $supporderproduct->productunite->id]); ?>
                                    <?= $this->Form->control('supporderproducts.' . $supporderproduct->id . '.qtepercs', ['type' => 'hidden', 'class' => 'form-control', 'label' => false, 'value' => $supporderproduct->productunite->quantity]); ?>
                                    <td style="width: 50%;">
                                        <?= $supporderproduct->product->title ?><br>
                                    </td>
                                    <td>
                                        <?= $this->Form->control('supporderproducts.' . $supporderproduct->id . '.quantity', ['type' => 'number', 'min' => '0', 'class' => 'form-control', 'label' => false, 'value' => intVal($supporderproduct->quantity / $supporderproduct->productunite->quantity)]); ?>
                                    </td>
                                </tr>
                            <?php endforeach ?>
                        </tbody>
    ...
    'ajax': {
    'url' :
    "<?php echo $this->Url->build(['controller' => 'Receipts', 'action' => 'instanceord', $receipt->supplierorder_id]); ?>",
    },
    'columns': [
    { data: 'product'},
    { data: 'quantity'},
    ...
    tablev.ajax.url(
    '<?php echo $this->Url->build(['controller' => 'Receipts', 'action' => 'addedord', $receipt->id]); ?>' ).load();
