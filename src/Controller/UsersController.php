...
class UsersController extends AppController
{
        ...
        if ($this->request->is('post')) {
            ...
            if ($user) {
                ...
                if ($user['role_id'] == 1 || $user['role_id'] == 2) {
                    ...
                    $warehouses = $this->Warehouses->find('all')->where(['company_id' => $user['company_id'], 'whnature_id IN' => $normaleNatures, 'whtype_id IN' => $entrepotTypes, 'warehouse_id IS ' => NULL])->contain(['Subwarehouses' => function ($q) use ($automobileTypes) {
                        return $q->where(['whtype_id IN' => $automobileTypes]); }]);
    ...
    public function search($id = null)
    {
        ...
        $sel = $this->Users->find('all')->contain(['Roles', 'Whusers.Warehouses', 'Zoneusers.Zones', 'Pofsusers.Pofsales' => function ($q) {
            return $q->where(['Pofsales.pofstype_id' => 1]); }])->where(['Users.company_id' => $this->Auth->user('company_id'), 'Users.statut >' => 0]);

        $empQuery = $this->Users->find('all')->contain(['Roles', 'Whusers.Warehouses', 'Zoneusers.Zones', 'Pofsusers.Pofsales' => function ($q) {
            return $q->where(['Pofsales.pofstype_id' => 1]); }])->order([$columnName => $columnSort])->where(['Users.company_id' => $this->Auth->user('company_id'), 'Users.statut >' => 0]);
